...
type FeedItem = { id: string; imageUrl: string; added: boolean; raw: string }
...
export default function FeedGrid({ apiPath }: { apiPath: string }) {
  ...
  const load = useCallback(async () => {
    const res = await fetch(`${apiPath}?limit=180`)
    if (!res.ok) return
  ...
  const save = useCallback(
    async (ids: string[]) => {
      ...
      setSelected(new Set())
      for (const id of pending) await add(id)
    },
    [display, add]
  )
  ...
  useGridKeys({
    ...
    onOpen: id => {
      ...
      if (item) window.open(`/?raw=${encodeURIComponent(item.raw)}`, '_blank')
    },
    onPause: () => {
      setPaused(p => {
