...
export default function RefsPage() {
  ...
  const { cellRefs, handleClick, handleMouseDown, selRect } = useDragSelect(keys, selected, setSelected)
  ...
  async function flush() {
    ...
    try {
      while (pendingRaws.current.size > 0) {
        ...
        const lines = raws.map(r => pendingLines.current.get(r) ?? -1).filter(l => l > 0)
      ...
      busy.current = false
      refetch()
      if (pendingRaws.current.size > 0) void flush()
    }
  }
  ...
  function enqueueDelete(raws: string[]) {
    ...
    for (const raw of raws) {
      if (!pendingLines.current.has(raw)) pendingLines.current.set(raw, lineByRaw.get(raw) ?? -1)
  ...
  async function saveEditor() {
    ...
    try {
      ...
      const res = await fetch('/api/refs', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ line: editLine, expectedRaw: editExpectedRaw, raw }),
      })
  ...
  function applyHash() {
    ...
    try { c = new URL(c).searchParams.get('c') || c } catch { /* url parse */ }
    if (c.startsWith('c=')) c = c.slice(2)
    if (!c) return toast('missing c hash')

    try {
      const flat = Object.fromEntries(Object.entries(decode(c)).map(([k, v]) => [k, v.value]))
        ...
        {refs.map((r, i) => (
          <div
            className={`group relative cursor-pointer ${
              selected.has(r.raw) ? 'ring-2 ring-blue-500' : 'ring-1 ring-neutral-800'
            }`}
            key={r.raw}
            ...
            ref={el => { el ? cellRefs.current.set(r.raw, el) : cellRefs.current.delete(r.raw) }}
          >
            <div className="aspect-square w-full bg-neutral-900">
              ...
              <img
                ...
                src={r.thumbUrl && !thumbFails.has(r.raw) ? r.thumbUrl : rasterUrl(r.params)}
              />
            </div>
            ...
            <button
              ...
              data-no-drag
              onClick={e => { e.stopPropagation(); enqueueDelete([r.raw]) }}
            >
              x
            </button>
            ...
            <button
              ...
              onClick={e => {
                e.stopPropagation()
                try { setEditText(JSON.stringify(JSON.parse(r.raw), null, 2)) } catch { setEditText(r.raw) }
                setEditLine(r.line)
        ...
        <div className="fixed inset-0 z-50 bg-black/70 p-4">
          <div className="mx-auto flex h-full max-w-2xl flex-col border border-neutral-700 bg-neutral-900 p-3">
            ...
            <div className="flex items-center justify-between border-b border-neutral-700 px-3 py-2 text-xs text-neutral-300">
              <span className="font-['Courier_New',monospace]">edit line {editLine}</span>
              <button className="bg-neutral-700 px-2 py-1 hover:bg-neutral-600" onClick={() => setEditOpen(false)}>close</button>
            </div>
            ...
            <div className="grid grid-cols-[1fr_auto] gap-2 border-b border-neutral-700 p-2">
              <input
                ...
                value={hashInput}
              />
              <button className="bg-neutral-700 px-3 py-1 text-xs hover:bg-neutral-600" onClick={applyHash}>from c</button>
            </div>
            ...
            <div className="flex justify-end gap-2 border-t border-neutral-700 px-3 py-2">
              <button className="bg-neutral-700 px-3 py-1.5 text-xs hover:bg-neutral-600" onClick={() => setEditOpen(false)}>cancel</button>
              <button
