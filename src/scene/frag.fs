...
void main() {
  ...
  for (int i = 0; i < uSteps; i++) {
    ...
    float d1 = sdRoundedBox(p1, vec2(r) / scale, vec4(r));
    d1 = opSmoothUnion(d1, sdHeart(p1), 0.1);
    d1 = U(d1) * alpha;

    vec2 p2 = uv * rot(radians(angle * -.4));
    float d2 = sdRoundedBox(p2, vec2(r) / scale, vec4(r));
    d2 = opSmoothUnion(d2, sdSegment(p2, vec2(-.5, .5), vec2(.5, -.5)), .5);
    d2 = U(d2) * alpha;
